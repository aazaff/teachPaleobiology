...
2) What is oldest age of each genus? [[Hint](https://raw.githubusercontent.com/aazaff/startLearn.R/master/intermediateConcepts.md#L244-255): Use the ````tapply(  )```` and ````max(  )```` functions we've used in previous labs]. Show the code you would use to find out.
